...
std::array<int, 81> Solver::generateSolvedBoard()
{
    ...
    // Try to generate with randomized backtracking
    if (solveBacktrack(board, true))
        return board;

    // fallback: return zeros if generation fails
...
std::array<int, 81> Solver::generatePuzzle(int clues)
{
    ...
    // clamp clues between a reasonable range (17 is often cited as minimal)
...
bool Solver::solveBacktrack(std::array<int, 81>& board, bool randomize)
{
    ...
    if (randomize)
    {
        // deterministic-ish seed based on time for varied boards
