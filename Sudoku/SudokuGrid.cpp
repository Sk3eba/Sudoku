...
SudokuGrid::SudokuGrid(sf::Vector2f position, float size)
    : m_position(position)
    , m_size(size)
    , m_cellSize(size / 9.f)
{
...
void SudokuGrid::draw(sf::RenderTarget &target) const
{
...
void SudokuGrid::setPosition(sf::Vector2f position)
{
    m_position = position;
    rebuildShapes();
}

sf::Vector2i SudokuGrid::cellAt(sf::Vector2f point) const {
...
sf::FloatRect SudokuGrid::getCellBounds(sf::Vector2i cell) const
{
...
void SudokuGrid::selectCell(sf::Vector2i cell) {
    if (cell.x >= 0)
    {
